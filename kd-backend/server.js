...
// Hilfsfunktion: Spaltenindex finden
...
// Hilfsfunktion: Prüfen ob User Gov ID existiert (in Users Tabelle)
...
// Hilfsfunktion: Prüfen ob Gov ID in hochgeladenen Dateien existiert
...
// Hilfsfunktion: Kingdom per Slug finden
...
function authenticateToken(req, res, next) {
  ...
  jwt.verify(token, JWT_SECRET, async (err, user) => {
    ...
    // Hole aktuelle User-Daten inkl. KingdomID aus der DB
...
app.get('/api/debug/users', async (req, res) => {
  try {
    const users = await all('SELECT id, username, email, is_approved, role, governor_id, kingdom_id FROM users');
    res.json(users);
  } catch (error) {
...
app.post('/api/admin/users/approve', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    if (currentUserKingdomId) {
        ...
        if (currentUserRole !== 'admin' && target.role === 'admin') return res.status(403).json({ error: 'Kein Zugriff auf Admin' });
    }

    const result = await query('UPDATE users SET is_approved = $1 WHERE id = $2', [approved, userId]);
    if (result.rowCount === 0) return res.status(404).json({ error: 'Benutzer nicht gefunden' });
...
app.post('/api/admin/users/role', authenticateToken, requireAdmin, async (req, res) => {
    try {
      ...
      // Wenn R5/Admin eine Rolle (nicht-admin) vergibt, Kingdom-ID sicherstellen
...
app.delete('/api/admin/kingdoms/:id', authenticateToken, requireAdmin, async (req, res) => {
    ...
    try {
        ...
        if (k && k.slug === 'default-kingdom') return res.status(400).json({ error: 'Default Kingdom protected' });
...
app.post('/overview/files/reorder', authenticateToken, async (req, res) => {
    ...
    if (!order || !Array.isArray(order)) return res.status(400).json({ error: 'Invalid order' });
