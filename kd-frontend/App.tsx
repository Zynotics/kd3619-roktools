...
const AppContent: React.FC = () => {
  ...
  // 🛡️ SICHERE ROLLEN-PRÜFUNG
  // Wir normalisieren die Rolle auf Kleinbuchstaben und entfernen Leerzeichen
  const safeRole = user?.role ? String(user.role).trim().toLowerCase() : '';
  const isSuperAdmin = safeRole === 'admin';
  const isR5 = safeRole === 'r5';
  const isR4 = safeRole === 'r4';
  const isAdmin = isSuperAdmin || isR5;
  // Bedingung für Activity View:
  const canSeeActivity = isSuperAdmin || isR5 || isR4;
      ...
      if (publicSlug) {
        try {
          ...
          if (res.ok) {
            ...
            setHeaderTitle(data.displayName || publicSlug.toUpperCase());
          } else {
            setHeaderTitle(`Kingdom ${publicSlug}`);
          }
        } catch { setHeaderTitle(`Kingdom ${publicSlug}`); }
        return;
      }
      if (user?.kingdomId) {
        ...
        <div className="w-full max-w-md">
          ...
          <div className="mt-8 text-center text-gray-500 text-sm">
             {isRegistrationInviteView ? (
                 <>
                    <p>Please login or register to access this kingdom.</p>
                    <button onClick={() => {
                        const newUrl = new URL(window.location.href);
                        newUrl.searchParams.delete('register');
                        window.location.href = newUrl.toString();
                    }} className="text-blue-400 hover:text-blue-300 mt-2 underline">View Public Dashboard</button>
                 </>
             ) : (
                <p>Access requires a valid link or login.</p>
             )}
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <aside className="hidden lg:fixed lg:inset-y-0 lg:flex lg:w-64 lg:flex-col bg-gray-900 border-r border-gray-800 z-50">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-6 space-y-2">
            <NavItem view="overview" currentActiveView={activeView} setActiveView={setActiveView} label="Analytics"
            ...
            />
            <NavItem view="honor" currentActiveView={activeView} setActiveView={setActiveView} label="Honor"
            ...
            />
            <NavItem view="analytics" currentActiveView={activeView} setActiveView={setActiveView} label="Players"
              icon={<svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" /></svg>}
            />

            {/* 🆕 Weekly Activity Nav Item */}
            {canSeeActivity && (
              <NavItem
                view="activity"
                currentActiveView={activeView}
            ...
            {isSuperAdmin && (
              <div className="pt-4 mt-4 border-t border-gray-800">
                ...
                <NavItem view="admin" currentActiveView={activeView} setActiveView={setActiveView} label="User Management"
                ...
                <div className="flex flex-col space-y-3">
                    <div className="flex items-center">
                        <div className="ml-0">
                            ...
                            <p className="text-xs text-gray-500 capitalize">{safeRole}</p>
                        </div>
                    </div>
                    {/* DEBUG ANZEIGE */}
                    <div className="text-[10px] bg-red-900/50 text-red-200 p-1 rounded border border-red-800">
                        <p>Rolle raw: "{user.role}"</p>
                        <p>Safe: "{safeRole}"</p>
                        <p>CanSeeActivity: {canSeeActivity ? 'YES' : 'NO'}</p>
                    </div>
                    {/* ENDE DEBUG */}
                    <button onClick={logout} className="w-full flex items-center justify-center px-4 py-2 border border-gray-700 rounded-md shadow-sm text-xs font-medium text-gray-300 bg-gray-800 hover:bg-gray-700 focus:outline-none">Log out</button>
                </div>
      ...
      <div className="lg:pl-64 flex flex-col min-h-screen">
        ...
        <header className="lg:hidden sticky top-0 z-40 flex h-16 items-center gap-x-4 border-b border-gray-800 bg-gray-900 px-4 shadow-sm">
             <div className="h-8 w-8 rounded-lg bg-blue-600 flex items-center justify-center"><span className="font-bold text-white text-xs">KD</span></div>
             <div className="flex-1 text-sm font-bold text-white truncate">{headerTitle}</div>
             {user && (<button onClick={logout} className="text-gray-400 hover:text-white"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1" /></svg></button>)}
        </header>
        ...
        <main className="flex-1 py-8 px-4 sm:px-6 lg:px-8 bg-black/20">
            <div className="max-w-7xl mx-auto">
                ...
                {/* DEBUG BANNER MOBILE (optional) */}
                <div className="lg:hidden mb-4 p-2 bg-red-900/50 text-xs text-red-200 border border-red-800 rounded">
                    DEBUG: Rolle="{user?.role}", Activity={canSeeActivity ? 'YES' : 'NO'}
                </div>
                ...
                {activeView === 'activity' && canSeeActivity && (
                    <ProtectedRoute accessType="overview"> 
                        <ActivityDashboard isAdmin={!!isAdmin} backendUrl={BACKEND_URL} />
                    </ProtectedRoute>
                )}
...
const PublicOrProtectedRoute: React.FC<PublicOrProtectedRouteProps> = ({ children, isPublic, publicSlug, accessType, isAdminOverride }) => {
  if (isPublic || isAdminOverride) {
    if (!publicSlug && isPublic) return <div className="text-center p-8 text-red-400 bg-gray-800 rounded-xl mt-10">Invalid Public Link.</div>;
    return <>{children}</>;
  }
