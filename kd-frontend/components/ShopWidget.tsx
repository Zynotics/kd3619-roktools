...
const PRODUCTS: Product[] = [
  { id: '1d', label: '1 Tag Zugang', price: 5, description: 'Kurztest für einen Tag.' },
  ...
  { id: '1y', label: '1 Jahr Zugang', price: 220, description: 'Jahreslizenz für Power User.' },
];
...
const ShopWidget: React.FC<ShopWidgetProps> = ({ kingdomSlug }) => {
  ...
  return (
    <div className="fixed bottom-6 right-4 z-50 max-w-sm w-full text-white">
      <div className={`bg-gray-900/95 border border-blue-700 rounded-2xl shadow-2xl overflow-hidden ${isOpen ? '' : 'pointer-events-none'}`}>
        <div className="flex items-center justify-between px-4 py-3 bg-blue-700/30 backdrop-blur-sm border-b border-blue-700">
          <div>
            <p className="text-xs uppercase tracking-widest text-blue-200">KD Shop</p>
            <p className="text-sm font-semibold text-white">Zugänge immer sichtbar kaufen</p>
          </div>
          <button
          ...
          <div className="p-4 space-y-4 max-h-[65vh] overflow-y-auto">
            <div className="grid grid-cols-1 gap-3">
              {PRODUCTS.map(product => (
                <div
                  key={product.id}
                  className="bg-gray-800/80 border border-gray-700 rounded-xl p-3 flex items-start justify-between gap-3"
                >
                  <div>
            ...
            <div className="bg-black/30 border border-gray-700 rounded-xl p-4 space-y-3">
              ...
              {cart.length === 0 ? (
                <p className="text-xs text-gray-500">Keine Artikel hinzugefügt.</p>
              ) : (
                  ...
                  {cart.map(item => (
                    <div key={item.id} className="flex items-center justify-between text-sm">
                      <div>
                        ...
                        <p className="text-xs text-gray-500">${item.price} / Lizenz</p>
                      </div>
              ...
              <div className="space-y-2">
                {!user && (
                  <div className="text-xs text-amber-300 bg-amber-500/10 border border-amber-600 rounded-lg p-2">
                    Bitte einloggen oder registrieren um den Checkout abzuschließen.
                  </div>
                )}
                {checkoutMessage && (
