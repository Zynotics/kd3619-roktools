...
export const Table: React.FC<TableProps> = ({ children, className = '', maxHeight = 'auto' }) => {
  return (
    <div className={`overflow-x-auto relative border border-gray-700 rounded-lg ${maxHeight !== 'auto' ? `max-h-${maxHeight}` : ''}`}>
...
export const TableHeader: React.FC<TableHeaderProps> = ({ children, sticky = true }) => {
  return (
    <thead className={`text-xs text-gray-400 uppercase bg-gray-700 ${sticky ? 'sticky top-0' : ''}`}>
      {children}
    </thead>
  );
};
...
export const TableRow: React.FC<TableRowProps> = ({ children, hover = true, className = '' }) => {
...
export const TableCell: React.FC<TableCellProps> = ({ 
  ...
  return (
    <td className={`px-4 py-3 whitespace-nowrap ${alignClass} ${textClass} ${className}`}>
      {children}
    </td>
  );
};