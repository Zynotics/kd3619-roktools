...
import type { UploadedFile, PlayerInfo, PlayerAnalyticsHistory } from '../types';
...
const PowerAnalyticsDashboard: React.FC<PowerAnalyticsDashboardProps> = ({ isAdmin, backendUrl }) => {
  ...
  const playerAnalyticsHistories = useMemo(() => {
    ...
    uploadedFiles.forEach((file) => {
      const players = file.data.map((row) => {
      ...
      players.forEach((player) => {
        ...
        historyEntry.history.push({
          ...
          totalKills: player.t1Kills + player.t2Kills + player.t3Kills + player.t4Kills + player.t5Kills,
        });
      });
    });
  ...
  const allPlayersLatest = useMemo(() => {
    const latestPlayers = new Map<string, PlayerInfo>();
    uploadedFiles.forEach((file) => {
      const players = file.data.map((row) => {
        const getVal = (keywords: string[]) => {
          const index = findColumnIndex(file.headers, keywords);
      ...
      players.forEach((player) => {
        latestPlayers.set(player.id, player);
      });
    });
  ...
  if (isLoading) {
    return <div className="text-center p-8 text-gray-400">Loading analytics data...</div>;
  }
